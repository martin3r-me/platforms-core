<div x-data="{ moduleFlyoutOpen: false, search: '' }"
    ...
    <template x-teleport="body">
        <div x-show="moduleFlyoutOpen" x-cloak x-transition x-ref="panel"
            ...
            x-effect="if(moduleFlyoutOpen){ $nextTick(() => { const r = $refs.trigger.getBoundingClientRect(); $el.style.top = (r.bottom + 8) + 'px'; const left = Math.min(r.left, window.innerWidth - $el.offsetWidth - 8); $el.style.left = Math.max(8, left) + 'px'; $refs.moduleSearch?.focus(); }) }"
            class="fixed z-[99] max-h-[80vh] w-[720px] max-w-[92vw] overflow-y-auto rounded-[8px] border border-[color:var(--nx-line)] bg-[color:var(--nx-surface)] p-3 shadow-[var(--nx-shadow-pop)]">
            ...
            {{-- Dashboard (volle Breite, über den Spalten) --}}
            <a href="{{ route('platform.dashboard') }}"
                x-show="!search || 'dashboard haupt-dashboard'.includes(search.toLowerCase())"
                class="mod-item mb-2 flex items-center gap-2 rounded-[6px] px-2 py-1 text-sm text-[color:var(--nx-text)] transition-colors hover:bg-[color:var(--nx-hover)]">
                @svg('heroicon-o-home', 'w-4 h-4 shrink-0 text-[color:var(--nx-muted)]')
                <span class="truncate">Haupt-Dashboard</span>
            </a>

            {{-- Gruppierte Module — Gruppen bleiben als Block zusammen, fließen in Spalten --}}
            <div class="gap-x-4 [column-fill:balance] sm:columns-2 lg:columns-3">
                @foreach($groupedModules as $groupKey => $group)
                    <div x-data="{ hasVisible: false }"
                         @class([
                            'mb-4 break-inside-avoid',
                            'border-t border-[color:var(--nx-line)] pt-2' => $groupKey === 'admin',
                         ])
                        ...
                        <h3 x-show="hasVisible" class="mb-1 px-2 text-[11px] font-semibold uppercase tracking-wide text-[color:var(--nx-faint)]">
                            {{ $group['label'] }}
                        </h3>
                        <div class="space-y-0.5">
                            @foreach($group['modules'] as $module)
                                ...
                                <a href="{{ $finalUrl }}"
                                    ...
                                    class="mod-item flex items-center gap-2 rounded-[6px] px-2 py-1 text-sm transition-colors hover:bg-[color:var(--nx-hover)] {{ $isCurrent ? 'bg-[color:var(--nx-accent-soft)] font-medium text-[color:var(--nx-text)]' : 'text-[color:var(--nx-text)]' }}">
                                    @if(!empty($icon))
