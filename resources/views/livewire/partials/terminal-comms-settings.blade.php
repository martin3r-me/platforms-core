...
<div class="flex-1 min-h-0 flex flex-col">
  ...
  {{-- Header --}}
  <div class="px-4 py-2.5 border-b border-[var(--t-border)]/40 flex items-center justify-between flex-shrink-0 bg-white/[0.02]">
    ...
    <button wire:click="closeCommsSettings" class="w-6 h-6 rounded-md flex items-center justify-center text-[var(--t-text-muted)] hover:text-[var(--t-text)] hover:bg-white/10 transition">
  ...
  <div class="flex-1 min-h-0 overflow-y-auto p-4 space-y-5">
      ...
      <div class="p-4 space-y-3">
        @if($postmarkMessage)
          <div class="text-[10px] text-emerald-400 font-medium"
               x-data="{ show: true }" x-init="setTimeout(() => { show = false; $wire.set('postmarkMessage', null); }, 4000)"
        ...
        <div class="flex justify-end">
          <button wire:click="savePostmarkConnection" wire:loading.attr="disabled" wire:target="savePostmarkConnection"
                  @if(!$this->canManageProviderConnections()) disabled @endif
            ...
            <span wire:loading.remove wire:target="savePostmarkConnection">Speichern</span>
            <span wire:loading wire:target="savePostmarkConnection">Speichert...</span>
          </button>
        </div>
        ...
        <div class="border-t border-[var(--t-border)]/20 pt-3 space-y-2">
          ...
          @if($postmarkDomainMessage)
            <div class="text-[10px] text-emerald-400 font-medium"
                 x-data="{ show: true }" x-init="setTimeout(() => { show = false; $wire.set('postmarkDomainMessage', null); }, 4000)"
                 x-show="show" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
    ...
    <div class="rounded-xl border border-[var(--t-border)]/25 bg-white/[0.02] overflow-hidden">
      ...
      <div class="p-4 space-y-3">
        @if($channelsMessage)
          <div class="text-[10px] text-emerald-400 font-medium"
               x-data="{ show: true }" x-init="setTimeout(() => { show = false; $wire.set('channelsMessage', null); }, 4000)"
               x-show="show" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
            {{ $channelsMessage }}
          </div>
        @endif
        ...
        <div class="rounded-lg border border-dashed border-[var(--t-border)]/30 bg-white/[0.01] p-3 space-y-2.5">
          ...
          @if($newChannel['type'] === 'email')
            ...
            <div class="flex justify-end">
              <button wire:click="createChannel" wire:loading.attr="disabled" wire:target="createChannel"
                ...
                <svg class="w-3 h-3" wire:loading.remove wire:target="createChannel" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/></svg>
                <svg class="w-3 h-3 animate-spin" wire:loading wire:target="createChannel" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                E-Mail Kanal
              </button>
            </div>
          @else
