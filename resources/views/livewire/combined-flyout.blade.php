<div x-data="{ combinedFlyoutOpen: false }"
    ...
    <button x-ref="trigger" @click="combinedFlyoutOpen = !combinedFlyoutOpen"
    ...
    <template x-teleport="body">
        <div x-show="combinedFlyoutOpen" x-cloak x-transition
            @click.outside="combinedFlyoutOpen = false"
            x-effect="if(combinedFlyoutOpen) { $nextTick(() => { const r = $refs.trigger.getBoundingClientRect(); $el.style.top = (r.bottom + 8) + 'px'; $el.style.right = (window.innerWidth - r.right) + 'px'; }) }"
            class="fixed z-[99] w-screen max-w-4xl bg-[var(--ui-surface)] rounded-2xl border border-[var(--ui-border)]/60 shadow-lg">
            <div class="p-6">
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            @foreach($userTeams as $team)
                                ...
                                @else
                                    <button type="button"
                                        @click="$wire.switchTeam({{ $team->id }}); combinedFlyoutOpen = false"
