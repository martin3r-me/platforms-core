    ...
    <div class="flex-1 min-h-0 flex"
      ...
      <div class="flex-1 min-w-0 flex flex-col" wire:key="terminal-main-{{ $channelId }}">
        ...
        @if($this->activeChannel)
          ...
          <!-- ═══ App: Aktivitäten ═══ -->
          <template x-if="activeApp === 'activity'">
          <div class="flex-1 min-h-0 overflow-y-auto overscroll-contain">
            <div class="py-4" :class="fullscreen ? 'px-6' : 'px-4'">
              ...
              <template x-if="!activitiesLoading && contextActivities.length === 0">
                <div class="py-8 text-center">
                  <div class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-[var(--ui-muted)]/5 mb-3">
                    <svg class="w-6 h-6 text-[var(--ui-muted)]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
                  </div>
              ...
              <template x-if="!activitiesLoading && contextActivities.length > 0">
                <div class="space-y-2">
                  <template x-for="act in contextActivities" :key="act.id">
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-surface-hover)]/30 hover:bg-[var(--ui-surface-hover)]/60 transition-colors">
                      <div class="flex items-start justify-between gap-2 mb-1">
                        <div class="flex-1 min-w-0 text-sm text-[var(--ui-body-color)] leading-snug" x-text="act.title"></div>
                        <template x-if="act.type === 'system'">
          ...
          <template x-if="activeApp === 'files'">
          <div class="flex-1 min-h-0 flex items-center justify-center text-[var(--ui-muted)] text-sm">
  ...
  <script>
    function terminalShell(){
      ...
      return {
        ...
        switchApp(app) {
          this.activeApp = app;
          if (app === 'activity') this.loadActivities();
          if (app === 'chat') {
            this.$nextTick(() => {
        ...
        loadActivities() {
          ...
          $wire.getContextActivities().then(r => {
            this.contextActivities = r;
