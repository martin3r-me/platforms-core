...
<div class="flex justify-center items-start pt-20 min-h-screen">
    <div class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
        <h2 class="text-2xl font-bold mb-6 text-center text-[var(--ui-primary)]">Login</h2>
        ...
        @if($policy->isPasswordLoginAllowed())
            <div class="text-center text-[var(--ui-secondary)] my-3"><small>oder</small></div>
        @endif

        {{-- SSO Button --}}
        <div class="grid gap-2">
            <a href="{{ route('azure-sso.login') }}"
               class="w-full border border-[rgb(var(--ui-primary-rgb))] text-[var(--ui-primary)] py-2 px-4 rounded hover:bg-[rgb(var(--ui-primary-rgb))] hover:text-[var(--ui-on-primary)] transition text-center">
        ...
        @if($policy->isManualRegistrationAllowed())
            <div class="mt-4 text-sm text-center text-[var(--ui-secondary)]">
                Noch kein Konto?
                <a href="{{ route('register') }}" class="text-[var(--ui-primary)] hover:underline">Registrieren</a>
            </div>
        @endif
    </div>
</div>