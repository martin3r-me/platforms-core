...
<x-ui-modal size="xl" wire:model="modalShow">
    <x-slot name="header">
        ...
        <div class="flex gap-1 mt-4 border-b border-gray-200">
            ...
            <button type="button" class="px-3 py-2 text-sm font-medium rounded-t-lg transition-colors" :class="{ 'text-blue-600 border-b-2 border-blue-600 bg-blue-50' : tab === 'ai-models', 'text-gray-500 hover:text-gray-700' : tab !== 'ai-models' }" @click="tab = 'ai-models'">AI Models</button>
    ...
    {{-- AI Models Tab --}}
    <div class="mt-6" x-show="tab === 'ai-models'" x-cloak>
        @php
            $isOwner = $this->canManageAiModels();
            $hasRecords = $this->hasTeamModelRecords();
        @endphp
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-semibold text-[var(--ui-secondary)]">AI-Modelle für Team</h3>
                <div class="flex items-center gap-2">
                    @if($hasRecords)
                        <span class="text-xs px-2 py-1 rounded bg-amber-100 text-amber-700">Team-Filter aktiv</span>
                        @if($isOwner)
                            <button
                                type="button"
                                class="text-xs px-2 py-1 rounded border border-[var(--ui-border)] bg-[var(--ui-bg)] hover:bg-[var(--ui-muted-5)] text-[var(--ui-muted)]"
                                wire:click="resetTeamModels"
                                title="Team-Filter zurücksetzen: alle Modelle wieder verfügbar"
                            >Reset Team-Filter</button>
                        @endif
                    @else
                        <span class="text-xs px-2 py-1 rounded bg-[var(--ui-muted-5)] text-[var(--ui-muted)]">Alle Modelle verfügbar</span>
                    @endif
                </div>
            </div>

            <p class="text-sm text-[var(--ui-muted)]">
                Hier legst du fest, welche AI-Modelle für dein Team und alle Kind-Teams verfügbar sind.
                @if(!$isOwner)
                    <span class="text-[var(--ui-danger)] font-semibold">Read-only (nur Root/Eltern-Team Owner darf ändern).</span>
                @endif
            </p>

            @if(!empty($this->teamModelMessage))
                <div class="text-xs px-3 py-2 rounded border border-[var(--ui-border)] bg-[var(--ui-bg)] text-[var(--ui-secondary)]">
                    {{ $this->teamModelMessage }}
                </div>
            @endif

            @if(count($this->teamAiModels) === 0)
                <div class="text-sm text-[var(--ui-muted)] p-4 bg-[var(--ui-muted-5)] rounded-lg">
                    Noch keine AI-Modelle in <code class="px-1 py-0.5 rounded bg-[var(--ui-muted-5)]">core_ai_models</code> vorhanden.
                </div>
            @else
                <div class="space-y-2">
                    @foreach($this->teamAiModels as $m)
                        @php
                            $enabled = $this->teamModelToggles[(int)$m->id] ?? true;
                        @endphp
                        <div class="flex items-center justify-between p-3 bg-[var(--ui-muted-5)] rounded-lg border border-[var(--ui-border)]/40">
                            <div class="flex items-center gap-3">
                                <div>
                                    <div class="font-semibold text-sm text-[var(--ui-secondary)]">{{ $m->name }}</div>
                                    <div class="text-xs text-[var(--ui-muted)]">
                                        {{ $m->provider?->key ?? '—' }} / {{ $m->model_id }}
                                    </div>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                @if($isOwner)
                                    <button
                                        type="button"
                                        class="text-xs px-3 py-1.5 rounded border border-[var(--ui-border)] font-medium transition-colors {{ $enabled ? 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100' : 'bg-red-50 text-red-700 border-red-200 hover:bg-red-100' }}"
                                        wire:click="toggleTeamModel({{ $m->id }})"
                                        title="{{ $enabled ? 'Modell für Team deaktivieren' : 'Modell für Team aktivieren' }}"
                                    >
                                        {{ $enabled ? 'Aktiv' : 'Gesperrt' }}
                                    </button>
                                @else
                                    @if($enabled)
                                        <span class="text-xs px-2 py-1 rounded bg-green-50 text-green-700">Aktiv</span>
                                    @else
                                        <span class="text-xs px-2 py-1 rounded bg-red-50 text-red-700">Gesperrt</span>
                                    @endif
                                @endif
                            </div>
                        </div>
